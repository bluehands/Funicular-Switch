...
{
    public static MyResult<IReadOnlyCollection<T1>> Map<T, T1>(this IEnumerable<MyResult<T>> results, Func<T, T1> map) =>
        results.Select(r => r.Map(map)).Aggregate();

    public static MyResult<IReadOnlyCollection<T1>> Bind<T, T1>(this IEnumerable<MyResult<T>> results, Func<T, MyResult<T1>> bind) =>
        results.Select(r => r.Bind(bind)).Aggregate();

    public static MyResult<IReadOnlyCollection<T1>> Bind<T, T1>(this MyResult<T> result, Func<T, IEnumerable<MyResult<T1>>> bindMany) =>
        result.Map(ok => bindMany(ok).Aggregate()).Flatten();

    public static MyResult<T1> Bind<T, T1>(this IEnumerable<MyResult<T>> results, Func<IEnumerable<T>, MyResult<T1>> bind) =>
        ...
        this IEnumerable<MyResult<T>> results)
    {
        ...
        return errors != null ? MyResult.Error<IReadOnlyCollection<T>>(errors) : MyResult.Ok<IReadOnlyCollection<T>>(oks);
    }
