...
static class Diagnostics
{
    ...
    public static Diagnostic InavlidGenerateMethodsForUsage(string message, Location location) =>
        Create(location,
            ...
            messageFormat: $"{message} -  Please use GenerateMethodsFor attribute with typeof expression like [assembly:GenerateMethodsFor(typeof(MyResult))].",
    ...
    public static Diagnostic InvalidResultTypeAttributeUsage(string message, Location location) =>
        Create(location,
            id: "FUN0001",
            title: "Invalid attribute usage",
            messageFormat: $"{message} -  ResultType attribute was not used with typeof expression like [ResultType(typeof(MyError))].",
            severity: DiagnosticSeverity.Error);
